...
import { normalizeBN, valueToBigNumber } from "./bignumber.mjs";
import { checkHash } from "./checkHash.mjs";
import {
  BasicProposalState,
  PayloadState,
...
function getProposalStepsAndAmounts({
  ...
  const isVotingStarted = proposalData.votingMachineState > VotingMachineProposalState.NotCreated;
  ...
  const predictPayloadExpiredTime = Math.max.apply(
    null,
    proposalData.payloads.map((payload) => {
      if ((payload == null ? void 0 : payload.state) && payload.state === PayloadState.Created) {
        return payload.expirationTime;
      } else if ((payload == null ? void 0 : payload.state) && payload.state === PayloadState.Queued) {
        return payload.queuedAt + payload.delay + payload.gracePeriod;
      } else {
        return 0;
      }
  ...
  const allPayloadsExecuted = proposalData.payloads.every(
    (payload) => (payload == null ? void 0 : payload.state) && payload.state === PayloadState.Executed
  );
  const allPayloadsCanceled = proposalData.payloads.every(
    (payload) => (payload == null ? void 0 : payload.state) && payload.state === PayloadState.Cancelled
  );
  const allPayloadsExpired = proposalData.payloads.every(
    (payload) => (payload == null ? void 0 : payload.state) && payload.state === PayloadState.Expired
  );
  const isCanceled = proposalData.basicState === BasicProposalState.Cancelled || allPayloadsCanceled;
  const isExpired = proposalData.basicState === BasicProposalState.Expired || allPayloadsExpired;
  ...
  const isProposalExecuted = isVotingEnded && isVotingClosed && !isVotingFailed && proposalData.basicState === BasicProposalState.Executed && !isCanceled;
  const isPayloadsQueued = isProposalExecuted && now > lastPayloadQueuedAt + executionPayloadTime;
  const isPayloadsExecuted = isVotingEnded && isVotingClosed && !isVotingFailed && proposalData.basicState === BasicProposalState.Executed && !isCanceled && allPayloadsExecuted && !isExpired;
...
function getEstimatedState(proposal, forVotesS, againstVotesS) {
  ...
  const isPayloadsWaitForQueued = proposal.data.basicState === BasicProposalState.Executed && now < lastPayloadQueuedAt + proposal.timings.executionPayloadTime;
...
function formatProposal(proposal) {
  ...
  const votingPowerBasic = proposal.balances.map((balance) => valueToBigNumber(balance.basicValue).toNumber()).reduce((sum, value) => sum + value, 0);
  const votingPower = proposal.balances.map((balance) => valueToBigNumber(balance.value).toNumber()).reduce((sum, value) => sum + value, 0);
...
//# sourceMappingURL=formatProposal.mjs.map