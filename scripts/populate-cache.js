...
var LowWithLodash = class extends Low {
  chain = import_lodash.default.chain(this).get("data");
};
...
var LowWithLodash2 = class extends Low {
  chain = import_lodash2.default.chain(this).get("data");
};
...
var LowWithLodash3 = class extends Low {
  chain = import_lodash3.default.chain(this).get("data");
};
...
var LowWithLodash4 = class extends Low {
  chain = import_lodash4.default.chain(this).get("data");
};
...
var LowWithLodash5 = class extends Low {
  chain = import_lodash5.default.chain(this).get("data");
};
    ...
    for (let i = 0; i < proposalsIds.length; i++) {
      ...
      if (proposalData) {
        ...
        const payloadsData = await Promise.all(
          payloadsChainIds.map(async (id2) => {
            return await Promise.all(
              payloadsControllers.map(async (controller) => {
                const proposalPayloadsIds = proposalData.initialPayloads.filter(
                  (payload) => payload.payloadsController === controller && payload.chainId === id2
                ).map((payload) => payload.id);
                const payloadController = appConfig.payloadsControllerConfig[id2].contractAddresses.some(
                  (address) => address === controller
                ) && controller;
                if (payloadController) {
                  const payloadsData2 = await payloadsControllerDataHelpers[id2].read.getPayloadsData([
                    controller,
                    proposalPayloadsIds
                  ]) || [];
